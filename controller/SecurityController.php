...
class SecurityController extends AbstractController
{
        ...
        if (Session::getUser()) {
            ...
            if ((Session::getUser()->getId()) || Session::isAdmin()) {
                ...
                if ($pseudo && $email) {
                    ...
                    $this->redirectTo("forum", "update", "updateProfil", $id);
                    exit;
                } else {
                    Session::addFlash("error", "Une erreur est survenue, réessayez ou assurez vous d'avoir les droits.");
                    $this->redirectTo("forum", "index");
    ...
    public function userIsBanned($id)
    {
        ...
        if ($user) {
            ...
            Session::addFlash("error", "Une erreur est survenue, assurez vous que l'utilisateur éxiste.");
            $this->redirectTo("forum", "index");
            exit;
        }
    }
    ...
    public function userIsNotBanned($id)
    {
        //permet de le pas injecter autre chose qu'un entier dans l'url
        $id = filter_input(INPUT_GET, "id", FILTER_VALIDATE_INT);
        //si l'id dans l'url n'éxiste pas alors
        ...
        if ($user) {
            // si l'user associé au topic est identique à l'user actuellement connecté à la session ou si l'admin est connecté alors
            if (Session::isAdmin()) {
                $userManager->isNotBanned($id);
                Session::addFlash("success", "Le membre est débanni !");
                $this->redirectTo("forum", "viewListUser");
                exit;
            } else {
                Session::addFlash("error", "Une erreur est survenue, réessayez ou assurez vous d'avoir les droits.");
